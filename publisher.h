...
public:
	...
	Publisher( const Publisher &obj ): addr_( obj.addr_ ),
					...
					fd( obj.fd ),
					recvNum( obj.recvNum ),
					destInfoMap( obj.destInfoMap ),
					recvEnable( obj.recvEnable ),
					recvCount( obj.recvCount )
	{
		std::cout<<"Publisher Class Copy Constructor ...."<<std::endl;
	...
	{
		std::cout<<"Publisher Class Assign Constructor ..."<<std::endl;		
		...
		fd = other.fd;
		recvNum = other.recvNum;
		destInfoMap = other.destInfoMap;
...
private:
	...
	int fd;
        std::vector<SubscribersInfo> destInfoMap;
        std::vector<bool> recvEnable;
        int recvNum;
...
template<typename T>
Publisher<T>::Publisher(): subscribersCount( 0 ), 
				size( 0 ),
				fd( -1 ),
				recvNum( 0 )
{
